...
function CategoryAdd() {
  ...
  useEffect(()=>{
    ...
    if(params.id>0)
    callApi('/admin/category_dtls',1,{comp_id:+comp,catg_id:+params.id});
  ...
  const imageSubmit= ()=>{
    ...
    if(catgName){
    comp = localStorage.getItem('comp_id')
    var file = new File([croppedImage], catgName.split(" ").join('')+".png");
  ...
  return (
    ...
    <section className="bg-white dark:bg-gray-900">
      <div className="py-8 px-4 mx-auto max-w-2xl lg:py-16">
        ...
        <div className="grid gap-4 sm:grid-cols-2 sm:gap-6">
          ...
          <div className="w-full">
            ...
            />
             {isCalled && !catgName? <div className='text-red-500 text-sm'>Name is required</div>:null}
          </div>
        </div>
    
      {croppedImage? <Image width={200} src={URL.createObjectURL(croppedImage)} className="w-40 h-40 object-cover rounded-lg"/>: !remoteImg.toString().includes(null)?<Image width={200} src={remoteImg} className="w-40 h-40 object-cover rounded-lg"/>:''} 
       {/* {croppedImage && }
       {remoteImg &&  */}
      {/* </div> */}
      ...
      <Dialog header="Cropper" maximizable visible={visible} style={{ width: '50vw',height:'35vw' }} onHide={() => setVisible(false)}>
    {/* <p className="m-0"> */}
