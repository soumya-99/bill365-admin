...
function Signin() {
  ...
  useEffect(()=>{
      if(response?.data?.suc==1 && response?.data?.msg.length>0 && called){
        ...
        localStorage.setItem('branch_address',response?.data?.msg[0].branch_address)
        ...
        localStorage.setItem('contact_person',response?.data?.msg[0].contact_person)
   ...
   callApi('/admin/user_login',1,{user_id:values.email,password:values.password});
    setCalled(true)
   }
   ...
   <img src={`${IMG}`} className='h-[368px] w-96'/>
    ...
    <div className='mt-24 mb-10 bg-white rounded-tl-lg rounded-bl-lg rounded-tr-lg rounded-br-lg sm:rounded-tr-lg sm:rounded-br-lg sm:rounded-bl-none sm:rounded-tl-none w-96 h-auto text-gray-600 p-8 font-semibold text-xl shadow-xl'>
      ...
      <div className='pt-6 block'>
        ...
        <input type="text" name='email' value={formik.values.email} className="block border-gray-300 text-[13px] pt-2 p-1 rounded-lg h- w-full border " placeholder="name@company.com" onChange={formik.handleChange} onBlur={formik.handleBlur}/>
      ...
      <div className='pt-6 block'>
        ...
        <input type="password" name='password' value={formik.values.password} className="block border-gray-300 text-[13px] pt-1 p-1 rounded-lg h- w-full border " placeholder="*******" onChange={formik.handleChange} onBlur={formik.handleBlur}/>
      ...
      <div className='pt-5 pb-4 block text-sm'>
        <button disabled={!formik.isValid} type="submit" className='bg-blue-900 hover:bg-blue-800 w-full text-white p-3 rounded-md disabled:bg-blue-400 dark:bg-white dark:text-white'>Sign In</button>
      </div>
      </form>
